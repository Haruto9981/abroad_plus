  ...
  describe "GET api/v1/diaries" do
    ...
    context "page を params で送信しない時" do
      ...
      it "1ページ目のレコード10件取得できる" do
        ...
        expect(res["diaries"][0].keys).to eq ["id", "title", "content", "image_url", "word_count", "created_at", "from_today", "user"]
    ...
    context "diary_id に対応する diaries レコードが存在する時" do
      ...
      context "diaries レコードのステータスが公開の場合" do
        ...
        it "正常にレコードを取得できる" do
          subject
          res = JSON.parse(response.body)
          expect(res.keys).to eq ["id", "title", "content", "image_url", "word_count", "created_at", "from_today", "user"]
